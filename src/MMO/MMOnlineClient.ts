...
import { MMEvents, ISaveContext, InventoryItem, Sword, Shield, Magic, MagicQuantities, IInventory, IOvlPayloadResult, LinkState, IMMCore } from './Core/MajorasMask/API/MMAPI';
...
import { Command } from './Core/MajorasMask/API/ICommandBuffer';
...
export class MMOnlineClient {
    ...
    @EventHandler(MMEvents.ON_ROOM_CHANGE)
    ...
    onInventoryUpdate(inventory: IInventory) {
        ...
        if (buf[0x4] !== InventoryItem.NONE && raw_inventory[buf[0x4]] !== InventoryItem.NONE) {
        ...
        if (buf[0x5] !== InventoryItem.NONE && raw_inventory[buf[0x5]] !== InventoryItem.NONE) {
        ...
        if (buf[0x6] !== InventoryItem.NONE && raw_inventory[buf[0x6]] !== InventoryItem.NONE) {
        ...
        if (!this.core.helper.isTitleScreen() && this.core.helper.isSceneNumberValid() && this.core.helper.isInterfaceShown()) {
            if (!this.core.helper.isPaused()) {
                ...
                if (this.LobbyConfig.data_syncing) {
                    ...
                    if (state === LinkState.BUSY || state === LinkState.GETTING_ITEM || state === LinkState.TALKING) this.clientStorage.needs_update = true;

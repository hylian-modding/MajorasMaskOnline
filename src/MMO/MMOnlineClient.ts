...
export class MMOnlineClient {
    ...
    onSceneContextSync_client(packet: MMO_ClientSceneContextUpdate) {
        ...
        let buf1: Buffer = this.core.save.liveSceneData_chests;
        if (Object.keys(parseFlagChanges(packet.chests, buf1) > 0)) {
            this.core.save.liveSceneData_chests = buf1;
        }

        let buf2: Buffer = this.core.save.liveSceneData_switch;
        ...
        let buf4: Buffer = this.core.save.liveSceneData_clear;
        if (Object.keys(parseFlagChanges(packet.clear, buf4) > 0)) {
            this.core.save.liveSceneData_clear = buf4;
        }

        let buf5: Buffer = this.core.save.liveSceneData_temp;

...
import { MMEvents, ILink, IMMHelper, IGlobalContext, ISaveContext, IActorManager, IMMCore, IOvlPayloadResult } from "./API/MMAPI";
import { GlobalContext } from "./GlobalContext";
import { SaveContext } from "./SaveContext";
import { CommandBuffer } from "./CommandBuffer";
import { MMOffsets } from "./API/MMOffsets";
import { ActorManager } from "./ActorManager";
import { Command } from "./API/ICommandBuffer";
import { PayloadType } from "modloader64_api/PayloadType";
import IMemory from "modloader64_api/IMemory";
import fs from 'fs';
import path from 'path';

export class MajorasMask implements ICore, IMMCore {
    ...
    preinit() {
        ...
        global.ModLoader["offsets"]["link"] = {} as MMOffsets;
        let offsets: MMOffsets = global.ModLoader["offsets"]["link"];


        global.ModLoader["offsets"] = {};
        global.ModLoader["offsets"]["link"] = new MMOffsets();
    ...
    onTick() {
        ...
        if (this.save.form !== this.last_known_age) {
            this.last_known_age = this.save.form;
            bus.emit(MMEvents.ON_AGE_CHANGE, this.last_known_age);
        }
...
export class OverlayPayload extends PayloadType {
    ...
    constructor(ext: string, logger: ILogger, core: IMMCore) {
        super(ext);
        this.logger = logger;
        this.core = core;
    }

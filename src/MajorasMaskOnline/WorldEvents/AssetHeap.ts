...
import { Z64Emote_Emote, Z64OnlineEvents } from "@MajorasMaskOnline/Z64OnlineAPI/Z64OnlineAPI";
...
export class AssetHeap {
    ...
    heap!: Heap | undefined;
    private name: string;
    private headerStart: number = 0x80F00040;
    private tableStart: number = 0x80F00490;
    ...
    preinit() {
        this.costumes = new Map<MMForms, Buffer[]>();
        ...
        if (fse.existsSync(this.assetPath)) {
            if (fse.lstatSync(this.assetPath).isDirectory()) {
                ...
                sb.writeBuffer(this.assetZip.toBuffer());
                while (sb.length % 0x10 !== 0) {
                    sb.writeUInt8(0);
                }
                fse.writeFileSync("./" + this.name + ".content", JSON.stringify({ data: sb.toBuffer().swap32() }));
            } else {
    ...
    onRomPatched(evt: any) {
        ...
        this.costumes.set(MMForms.HUMAN, []);
        ...
        this.assets.forEach((value: Buffer, key: string) => {
            ...
            if (key.indexOf("costumes/human") > -1) {
                Buffer.from(path.parse(key).name).copy(value, 0x5440);
                this.costumes.get(MMForms.HUMAN)!.push(this.assets.get(key)!);
    ...
    postinit() {
        this.ModLoader.emulator.rdramWriteBuffer(0x80F00000, this.ModLoader.utils.clearBuffer(this.ModLoader.emulator.rdramReadBuffer(0x80F00000, 1 * 1024 * 1024)));
        this.ModLoader.utils.setTimeoutFrames(() => {
            // Create the heap.
            this.heap = new Heap(this.ModLoader.emulator, 0x80F00000, 1 * 1024 * 1024);
            // Alloc header space.
            this.headerStart = this.heap.malloc(this.padNumber(0x4));
            // Reserve overlay space.
            let f = Buffer.alloc(1 * 1024, 0xFF);
            let ovl = this.heap.malloc(f.byteLength);
            this.ModLoader.emulator.rdramWriteBuffer(ovl, f);
